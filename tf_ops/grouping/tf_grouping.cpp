...
void queryBallPoint2Launcher(int b, int n, int m, int nsample, const float *xyz1, const float *xyz2, const float *radii, int *idx, int *pts_cnt);
class QueryBallPoint2GpuOp : public OpKernel {
    public:
        explicit QueryBallPoint2GpuOp(OpKernelConstruction* context) : OpKernel(context) {
            OP_REQUIRES_OK(context, context->GetAttr("nsample", &nsample_));
            OP_REQUIRES(context, nsample_ > 0, errors::InvalidArgument("QueryBallPoint2 expects positive nsample"));
        }
        ...
        void Compute(OpKernelContext* context) override {
            ...
            // Inputs
            const Tensor& xyz1_tensor = context->input(0);
            OP_REQUIRES(context, xyz1_tensor.dims()==3 && xyz1_tensor.shape().dim_size(2)==3, errors::InvalidArgument("QueryBallPoint2 expects (batch_size, ndataset, 3) xyz1 shape."));
            int b = xyz1_tensor.shape().dim_size(0);  // batch size
            int n = xyz1_tensor.shape().dim_size(1);  // number of input points

            const Tensor& xyz2_tensor = context->input(1);
            OP_REQUIRES(context, xyz2_tensor.dims()==3 && xyz2_tensor.shape().dim_size(2)==3, errors::InvalidArgument("QueryBallPoint2 expects (batch_size, npoint, 3) xyz2 shape."));
            int m = xyz2_tensor.shape().dim_size(1);  // number of sampled points

            const Tensor& radii_tensor = context->input(2);
            OP_REQUIRES(context, radii_tensor.dims()==2, errors::InvalidArgument("QueryBallPoint2 expects (batch_size, npoint) radii shape."));
            ...
            auto xyz2_flat = xyz2_tensor.flat<float>();
            const float *xyz2 = &(xyz2_flat(0));
            auto radii_flat = radii_tensor.flat<float>();
            const float *radii = &(radii_flat(0));

            auto idx_flat = idx_tensor->flat<int>();
            int *idx = &(idx_flat(0));
            auto pts_cnt_flat = pts_cnt_tensor->flat<int>();
            int *pts_cnt = &(pts_cnt_flat(0));
            queryBallPoint2Launcher(b, n, m, nsample_, xyz1, xyz2, radii, idx, pts_cnt);
        }
    private:
        int nsample_;
};
REGISTER_KERNEL_BUILDER(Name("QueryBallPoint2").Device(DEVICE_GPU), QueryBallPoint2GpuOp);
