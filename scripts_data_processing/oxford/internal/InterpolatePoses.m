function [poses, velocities] = InterpolatePoses(ins_file, pose_timestamps, origin_timestamp)
...
%   velocities: Interpolated velocities in N, E, D frame. Note that this is
%               not rotated regardless of the origin_timestamp.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if nargin < 3
    origin_timestamp = pose_timestamps(1);
  end
...
%%
  [lower_index_cols, lower_index_rows] = ...
      getLowerIndices(ins_timestamps, pose_timestamps);
  ...
  vel_lower = vel(lower_indices, :);
  vel_upper = vel(lower_indices+1, :);
  vel_interp = repmat((1-fractions),3,1)'.*vel_lower + ...
      repmat(fractions,3,1)'.*vel_upper;
  ...
  if nargin == 3
    poses_array = poses_array(1:4,1:4) \ poses_array;  % Make relative to first timestep
  end
  ...
  velocities = vel_interp(2:end, :);

end


function [lower_index_cols, lower_index_rows] = getLowerIndices(from, to)

    iTo = 1;
    
    assert(from(1) < to(1) && from(end) > to(end))
    
    lower_index_cols = zeros(length(to),1);
    
    for iFrom = 1 : length(from)
        
        if iTo > length(to)
            break
        end
        
        while iTo <= length(to) && to(iTo) < from(iFrom)
            lower_index_cols(iTo) = iFrom - 1;
            iTo = iTo + 1;
        end
        
    end
    
    lower_index_rows = (1 : length(to))';
end
