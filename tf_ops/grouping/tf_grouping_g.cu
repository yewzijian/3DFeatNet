...
// input: nsample (1), xyz1 (b,n,3), xyz2 (b,m,3), radii (b,n). n=number of points, m=number of clusters
// output: idx (b,m,nsample), pts_cnt (b,m)
...
void queryBallPoint2Launcher(int b, int n, int m, int nsample, const float *xyz1, const float *xyz2, const float *radii, int *idx, int *pts_cnt) {
    query_ball_point2_gpu<<<b,256>>>(b,n,m,nsample,xyz1,xyz2,radii,idx,pts_cnt);
    //cudaDeviceSynchronize();
}
